...
module.exports = function (pug) {
    ...
    pug.Lexer.prototype.code = function () {
        var captures;
        if (captures = /^(!?=|-)[ \t]*([^\n]+)/.exec(this.input)) {
    ...
    pug.Lexer.prototype.attrs = function() {
        if ('(' == this.input.charAt(0)) {
            ...
            if (equals === ':') {
                console.warn('`:` in pug is deprecated, please use `=`');
            }
    ...
    pug.Compiler.prototype.visitCode = function (code) {
        var val = code.val;
    ...
    pug.Compiler.prototype.attrs = function(attrs){
        var buf = []
          , classes = []
          , escaped = {}
